...
import { useOthersSyusai } from "../../../hooks/useFetchOthersData";
interface JapaneseProps {}

export const OthersSyusai: React.FC<JapaneseProps> = memo(() => {
  const { isOpen, onOpen, onClose } = useDisclosure();
  const { getOthersSyusai, dishes, loading } = useAllMyDishes();
  const { data } = useOthersSyusai();
  const { onSelectDish, selectedDish } = useSelectDish();
  const { user } = useFetchUserData();

  const { searchedDishes, handleIngredientSearch } = useIngredientSearch("others-syusai", user?.id);
