...
export const OthersShirumono: React.FC<OthersProps> = memo(() => {
  ...
  const { getOthersShirumono, dishes, loading } = useAllMyDishes();
  const { data } = useOthersShirumono();
  const { onSelectDish, selectedDish } = useSelectDish();
  const { user } = useFetchUserData();

  const { searchedDishes, handleIngredientSearch } = useIngredientSearch("others-shirumono", user?.id);
