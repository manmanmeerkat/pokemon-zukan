...
import { useOthersFukusai } from "../../../hooks/useFetchOthersData";
...
export const OthersFukusai: React.FC<OthersProps> = memo(() => {
  const { isOpen, onOpen, onClose } = useDisclosure();
  const { getOthersFukusai, dishes, loading } = useAllMyDishes();
  const { data } = useOthersFukusai();
  const { onSelectDish, selectedDish } = useSelectDish();
  const { user } = useFetchUserData();

  const { searchedDishes, handleIngredientSearch } = useIngredientSearch("others-fukusai", user?.id);
